...
# --- 1. CONFIGURAÇÃO DA PÁGINA (TRAVADA) ---
...
with tab_cad:
    ...
    data_visita = c3.date_input("📅 DATA", value=date.today(), key="f_date")
    ...
    cap_digitada = t4.text_input("Capacidade BTU/h", value="0", key="f_cap")
    ...
    fluido = t7.selectbox("Fluido", ["R-410A", "R-32", "R-22", "R-134a", "R-404A"], key="f_gas")
...
with tab_diag:
    # MATRIZ TÉCNICA (LÓGICA INTERNA)
    diag_status = "Normal"
    if sh_val > 12 and sc_val < 3: diag_status = "Carga Insuficiente (Vazamento)"
    elif sh_val < 5 and sc_val > 8: diag_status = "Excesso de Carga"
    
    chk_sint = st.multiselect("Checklist de Sintomas:", ["Congelamento", "Ruído", "Erro Display", "Filtro Sujo"])
    resumo_pre = f"Diagnóstico: {diag_status} | Sintomas: {', '.join(chk_sint)}\nSH: {sh_val}K | SC: {sc_val}K\n\nParecer: "
    medidas = st.text_area("Parecer Tecnico e Recomendacoes", value=resumo_pre, height=150)
    
    if st.button("📄 Gerar Relatório Profissional Completo"):
        pdf = FPDF()
        pdf.add_page()
        
        # CABEÇALHO (MANTIDO CONFORME IMAGEM)
        pdf.image("logo.png", 10, 10, 40)
        pdf.set_xy(80, 10)
        pdf.set_font("Arial", 'B', 22); pdf.set_text_color(0, 51, 102)
        pdf.cell(50, 10, "MPN", 0, 1, 'C')
        pdf.set_x(80); pdf.set_font("Arial", 'B', 14); pdf.cell(50, 8, "Relatorio Tecnico", 0, 1, 'C')

        # 1. DADOS DO CLIENTE (MAPEAMENTO INDIVIDUAL)
        pdf.set_y(32)
        ...
        pdf.cell(90, 5, f"Cliente: {clean(cliente)}", 0, 0); pdf.cell(60, 5, f"CPF/CNPJ: {doc_cliente}", 0, 0)
        pdf.set_xy(170, pdf.get_y()-7); pdf.set_font("Arial", 'B', 7); pdf.cell(28, 4, f"DATA: {data_visita}", 1, 0, 'C'); pdf.set_font("Arial", '', 8)
        pdf.set_xy(12, pdf.get_y()+11); pdf.cell(110, 5, f"Endereco: {tipo_logr} {clean(nome_logr)}, {numero} ({clean(complemento)})", 0, 0); pdf.cell(60, 5, f"Bairro: {clean(bairro)}", 0, 1)
        pdf.set_x(12); pdf.cell(60, 5, f"CEP: {cep}", 0, 0); pdf.cell(90, 5, f"E-mail: {email_cli}", 0, 1)
        pdf.set_x(12); pdf.cell(50, 5, f"Whats: {whatsapp}", 0, 0); pdf.cell(50, 5, f"Cel: {celular}", 0, 0); pdf.cell(50, 5, f"Fixo: {tel_residencial}", 0, 1)

        # 2. DADOS DO EQUIPAMENTO (MAPEAMENTO INDIVIDUAL)
        ...
        pdf.set_x(12); pdf.cell(60, 4, f"Sistema: {tipo_eq}", 0, 0); pdf.cell(60, 4, f"Mod. Evap: {mod_evap}", 0, 0); pdf.cell(60, 4, f"Serie Evap: {serie_evap}", 0, 1)
        pdf.set_x(12); pdf.cell(60, 4, f"Mod. Cond: {mod_cond}", 0, 0); pdf.cell(60, 4, f"Serie Cond: {serie_cond}", 0, 0); pdf.cell(60, 4, f"Local Evap: {loc_evap}", 0, 1)
        ...
        pdf.set_xy(12, y_p+2); pdf.set_font("Arial", 'B', 8); pdf.cell(90, 4, "PARTE ELETRICA", 0, 0); pdf.cell(90, 4, "CICLO FRIGORIFICO", 0, 1)
        pdf.set_font("Arial", '', 8); pdf.set_text_color(0)
        pdf.set_x(12); pdf.cell(45, 4, f"Tensao Rede: {v_rede} V", 0, 0); pdf.cell(45, 4, f"Tensao Medida: {v_med} V", 0, 0); pdf.cell(45, 4, f"Pres. Succao: {p_suc} PSI", 0, 0); pdf.cell(45, 4, f"T-Sat Succao: {ts_suc} C", 0, 1)
        pdf.set_x(12); pdf.cell(45, 4, f"Corrente RLA: {rla_comp} A", 0, 0); pdf.cell(45, 4, f"Corrente Medida: {a_med} A", 0, 0); pdf.cell(45, 4, f"Pres. Descarga: {p_liq} PSI", 0, 0); pdf.cell(45, 4, f"T-Sat Liquido: {ts_liq} C", 0, 1)
        pdf.set_x(12); pdf.cell(90, 4, f"Carga do Motor: {carga_f} %", 0, 0); pdf.cell(45, 4, f"SH: {sh_val} K", 0, 0); pdf.cell(45, 4, f"SC: {sc_val} K", 0, 1)

        # 4. PARECER (ESTRUTURA ORIGINAL)
        ...
        st.download_button("⬇️ Baixar Relatório Final Lacrado", data=pdf.output(dest="S").encode('latin-1'), file_name=f"MPN_{cliente}.pdf")
