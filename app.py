...
def calcular_tsat_antoine(psig, gas, tipo="bubble"):
    ...
    if gas in coeficientes:
        ...
        A, B, C = params.get(tipo, params["bubble"]) if isinstance(params, dict) else params
        t_f = (B / (A - log_p)) - C
        return (t_f - 32) / 1.8
    return 0
...
with tab_cad:
    ...
    fluido_sel = c3.selectbox("Gás", ["R-410A", "R-22", "R-134a", "R-404A", "R-407C", "R-417A"], key="gas_ref")
...
with tab_termo:
    ...
    c_sat1.metric(f"Saturação Sucção ({f_ref})", f"{tsat_evap:.2f} °C")
    c_sat2.metric(f"Saturação Líquido ({f_ref})", f"{tsat_cond:.2f} °C")
...
with tab_diag:
    if st.button("Gerar Relatório PDF"):
        ...
        pdf.cell(190, 10, "LAUDO TÉCNICO MPN", ln=True, align="C")
        pdf.set_font("Arial", "", 12)
        pdf.ln(10)
        pdf.cell(190, 10, f"Gás Refrigerante: {f_ref}", ln=True)
        ...
        pdf.cell(190, 10, f"Superaquecimento: {sh:.1f} K", ln=True)
        pdf.cell(190, 10, f"Sub-resfriamento: {sr:.1f} K", ln=True)
