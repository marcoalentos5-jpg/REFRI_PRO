...
with tab_diag:
    ...
    if st.button("Gerar Relatório PDF"):
        ...
        # --- BLOCO TÉCNICO MANTIDO ---
        pdf.ln(5)
        pdf.set_font("Arial", "B", 11)
        pdf.cell(190, 8, " DADOS TÉCNICOS E MEDIÇÕES", ln=True, fill=True)
        pdf.set_font("Arial", "", 10)
        pdf.cell(95, 8, f"Fabricante: {fabricante}", border="B")
        pdf.cell(95, 8, f"Modelo Cond: {mod_cond}", border="B", ln=True)
        pdf.cell(63, 8, f"Superheat: {sh} K", border="B")
        pdf.cell(63, 8, f"Subcool: {sc} K", border="B")
        pdf.cell(64, 8, f"Delta T: {dt} K", border="B", ln=True)
        pdf.ln(5)
        pdf.set_font("Arial", "B", 11)
        pdf.cell(190, 8, " DIAGNÓSTICO", ln=True, fill=True)
        pdf.set_font("Arial", "", 10)
