...
with tab_diag:
    ...
    if st.button("📄 Gerar Relatório Profissional"):
        ...
        # Lógica Rigorosa de Fontes para evitar UnicodeException
        try:
            # Você precisa subir os arquivos .ttf para sua pasta do projeto
            pdf.add_font('DejaVu', '', 'DejaVuSans.ttf', uni=True)
            pdf.add_font('DejaVu', 'B', 'DejaVuSans-Bold.ttf', uni=True)
            font_name = 'DejaVu'
        except:
            font_name = 'Arial' # Fallback caso a fonte não exista
        ...
        pdf.cell(0, 15, "RELATÓRIO TÉCNICO PERICIAL", ln=True, align='C')
        
        pdf.set_text_color(0, 0, 0)
        pdf.set_font(font_name, 'B', 11)
        pdf.ln(25)
        pdf.cell(0, 8, f"CLIENTE: {cliente}", ln=True)
        pdf.cell(0, 8, f"LOCALIZAÇÃO: {local_eq}", ln=True)
        pdf.cell(0, 8, f"EQUIPAMENTO: {fabricante} {cap_digitada} BTU", ln=True)
        pdf.ln(5)
        pdf.set_font(font_name, '', 10)
        pdf.multi_cell(0, 8, f"DIAGNÓSTICO:\n{ia_raw}".encode('latin-1', 'replace').decode('latin-1') if font_name == 'Arial' else f"DIAGNÓSTICO:\n{ia_raw}")
        
        pdf_bytes = pdf.output(dest='S')
        if isinstance(pdf_bytes, str): pdf_bytes = pdf_bytes.encode('latin-1', 'replace')
        st.download_button("⬇️ Baixar PDF", data=pdf_bytes, file_name=f"Laudo_{cliente}.pdf")
