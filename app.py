...
# =========================================================
# 1. CONFIGURAÇÃO E DESIGN (ABAS 20PX)
# =========================================================
st.set_page_config(
...
# =========================================================
# 2. MOTOR DE DADOS E UTILITÁRIOS (CORREÇÃO DATABASE ERROR)
# =========================================================
def clean(txt):
...
def init_db():
    """Inicialização robusta: Cria ou atualiza a tabela"""
    conn = sqlite3.connect('banco_dados.db')
    c = conn.cursor()
    # Criamos a tabela com a estrutura exata exigida pelo app
    c.execute('''CREATE TABLE IF NOT EXISTS atendimentos (
        id INTEGER PRIMARY KEY AUTOINCREMENT, 
        data_visita TEXT, cliente TEXT, doc_cliente TEXT, 
        whatsapp TEXT, endereco TEXT, marca TEXT, modelo TEXT, 
        tecnologia TEXT, fluido TEXT, v_med REAL, a_med REAL, 
        p_suc REAL, p_liq REAL, sh REAL, sc REAL, 
        diagnostico TEXT, obs TEXT)''')
    conn.commit()
    conn.close()
...
def salvar_dados(dados):
    ...
    c.execute('''INSERT INTO atendimentos (
        data_visita, cliente, doc_cliente, whatsapp, endereco, 
        marca, modelo, tecnologia, fluido, v_med, a_med, 
        p_suc, p_liq, sh, sc, diagnostico, obs) 
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''', dados)
    conn.commit()
    conn.close()

def get_tsat(psig, gas):
    """Motor Termodinâmico Preciso"""
    tab = {
...
# Garante que o banco existe antes do app rodar
init_db()
...
# =========================================================
# 3. INTERFACE TÉCNICA (IDENTIFICAÇÃO E CÁLCULOS)
# =========================================================
with tab_cad:
    ...
    whatsapp = c4.text_input("WhatsApp (DDD + Número)", value="21")
...
with tab_ele:
    ...
    rla = el2.number_input("Corrente Nominal RLA (A)", value=1.0)
    
    percent_carga = (a_med / rla * 100) if rla > 0 else 0
    st.info(f"📊 Carga Atual do Compressor: {percent_carga:.1f}%")

with tab_termo:
    st.subheader("🌡️ Análise Ciclo Frigorífico")
    tr1, tr2, tr3 = st.columns(3)
    p_suc = tr1.number_input("Pressão Sucção (PSI)", value=118.0)
    t_suc_tubo = tr1.number_input("Temperatura Tubo Sucção (°C)", value=12.0)
    ts_suc = get_tsat(p_suc, fluido)
    sh_val = round(t_suc_tubo - ts_suc, 1)

    p_liq = tr2.number_input("Pressão Líquido (PSI)", value=345.0)
    t_liq_tubo = tr2.number_input("Temperatura Tubo Líquido (°C)", value=30.0)
    ts_liq = get_tsat(p_liq, fluido)
    sc_val = round(ts_liq - t_liq_tubo, 1)

    tr3.metric("Superaquecimento (SH)", f"{sh_val} K")
    tr3.metric("Subresfriamento (SC)", f"{sc_val} K")

# =========================================================
# 4. DIAGNÓSTICO, PDF E HISTÓRICO (PREVENÇÃO DE ERROS)
# =========================================================
with tab_diag:
    st.subheader("🤖 Diagnóstico Automático e Laudo")
    
    diag_ia = []
    if sh_val < 5: diag_ia.append("⚠️ Risco de retorno de líquido (SH Baixo)")
    elif sh_val > 12: diag_ia.append("⚠️ Possível falta de fluido ou restrição (SH Alto)")
    if sc_val < 5: diag_ia.append("⚠️ Condensação ineficiente ou baixa carga (SC Baixo)")
    if not diag_ia: diag_ia.append("✅ Sistema operando em equilíbrio ideal")
    
    resumo_ia = " | ".join(diag_ia)
    obs_tecnico = st.text_area("Observações Adicionais", placeholder="Relate detalhes aqui...")
    ...
    with c_btn1:
        ...
        st.markdown(f'<a href="https://wa.me/55{num_limpo}?text={msg}" target="_blank"><button style="width:100%; height:3.5em; background-color:#25D366; color:white; border:none; border-radius:8px; font-weight:bold; cursor:pointer;">🟢 Enviar via WhatsApp</button></a>', unsafe_allow_html=True)
    ...
    with c_btn2:
        if st.button("📑 Gerar Laudo PDF e Salvar"):
            ...
            pdf.cell(190, 15, clean("RELATÓRIO TÉCNICO MPN ENGENHARIA"), 1, 1, 'C', True)
            ...
            pdf.ln(5)
            pdf.set_font("Arial", 'B', 10)
            pdf.cell(190, 8, clean(" 2. ANÁLISE TÉCNICA"), 1, 1, 'L', True)
            pdf.set_font("Arial", '', 9)
            pdf.cell(95, 8, f"P.Suc: {p_suc} PSI", 1, 0); pdf.cell(95, 8, f"P.Liq: {p_liq} PSI", 1, 1)
            pdf.cell(95, 8, f"SH: {sh_val} K", 1, 0); pdf.cell(95, 8, f"SC: {sc_val} K", 1, 1)

            pdf.ln(10)
            pdf.multi_cell(190, 8, clean(f"DIAGNÓSTICO: {resumo_ia}"), 1)
            
            pdf_bytes = pdf.output(dest='S').encode('latin-1', 'ignore')
            st.download_button("📥 Baixar PDF do Laudo", data=pdf_bytes, file_name=f"Laudo_{cliente}.pdf")
...
with tab_hist:
    ...
    try:
        conn = sqlite3.connect('banco_dados.db')
        # Seleção explícita de colunas para evitar o DatabaseError
        df = pd.read_sql_query("SELECT data_visita, cliente, sh, sc, diagnostico FROM atendimentos ORDER BY id DESC", conn)
        st.dataframe(df, use_container_width=True, hide_index=True)
        conn.close()
    except Exception as e:
        st.warning("O histórico será exibido após o primeiro registro salvo com a nova estrutura.")
