# ==============================================================================
# 0. CONFIGURAÇÕES INICIAIS E IMPORTAÇÕES (VERSÃO INTEGRAL RESTAURADA)
# ==============================================================================
...
# 2. MOTOR DE SESSÃO (TODAS AS CHAVES ORIGINAIS SEM REDUÇÃO)
...
# --- FUNÇÕES DE MÁSCARA (AUSTE NA TUBULAÇÃO) ---
...
def formatar_telefone(num, fixo=False):
    num = re.sub(r'\D', '', num)
    if not fixo and len(num) == 11: return f"{num[:2]}-{num[2:3]}-{num[3:7]}-{num[7:]}"
    elif fixo and len(num) == 10: return f"{num[:2]}-{num[2:6]}-{num[6:]}"
    return num

def formatar_cep(cep):
    cep = re.sub(r'\D', '', cep)
    if len(cep) == 8: return f"{cep[:5]}-{cep[5:]}"
    return cep
...
# ==============================================================================
# 1. FUNÇÃO DA ABA 1: Identificação e Equipamento (CÓDIGO COMPLETO)
# ==============================================================================
def renderizar_aba_1():
    ...
    with tabs[0]:
        ...
        with st.expander("👤 Dados do Cliente e Endereço", expanded=True):
            ...
            # Máscara CPF
            cpf_in = c2.text_input("CPF ou CNPJ", value=st.session_state.dados['cpf_cnpj'])
            ...
            # LAYOUT ORIGINAL PRESERVADO (4 CAMPOS)
...
# ==============================================================================
# 4. LÓGICA DE EXIBIÇÃO E LOGO (CORRIGIDA)
# ==============================================================================
if aba_selecionada == "Home":
    st.markdown("<br>", unsafe_allow_html=True)
    c1, c2, c3 = st.columns([1, 2, 1]) 
    with c2: 
        # Busca exata pelo nome que você salvou
