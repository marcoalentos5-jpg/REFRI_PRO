...
def f_sat_precisao(p, g):
    """ Função que calcula a temperatura de saturação. 
    Precisa estar no topo para a Aba 2 não dar erro de 'NameError' """
    if p <= 5: return -50.0
    if g == "R410A":
...
def renderizar_aba_1():
    ...
    # 1. BLOCO DE ENDEREÇO (INDEXAÇÃO DO CEP)
    with st.expander("👤 Dados do Cliente e Endereço", expanded=True):
        c1, c2, c3 = st.columns([2, 1, 1])
        st.session_state.dados['nome'] = c1.text_input("Nome *", value=st.session_state.dados['nome'], key="n1")
        st.session_state.dados['cpf_cnpj'] = c2.text_input("CPF/CNPJ", value=st.session_state.dados['cpf_cnpj'], key="d1")
        st.session_state.dados['whatsapp'] = c3.text_input("WhatsApp *", value=st.session_state.dados['whatsapp'], key="w1")

        st.markdown("---")
        ce1, ce2, ce3 = st.columns([1, 2, 1])
        
        # Aqui o CEP salva e busca o endereço
        cep_input = ce1.text_input("CEP *", value=st.session_state.dados['cep'], key="cep_v3")
        if cep_input != st.session_state.dados['cep']:
            st.session_state.dados['cep'] = cep_input
            if buscar_cep(cep_input):
                st.rerun() # O segredo: ele recarrega a tela com a rua preenchida

        st.session_state.dados['endereco'] = ce2.text_input("Logradouro:", value=st.session_state.dados['endereco'], key="e1")
        st.session_state.dados['numero'] = ce3.text_input("Nº:", value=st.session_state.dados['numero'], key="num1")

        # Complemento, Bairro, Cidade e UF na mesma linha
        ca, cb, cc, cd = st.columns([1, 1, 1, 0.5])
        st.session_state.dados['complemento'] = ca.text_input("Compl.:", value=st.session_state.dados['complemento'], key="cp1")
        st.session_state.dados['bairro'] = cb.text_input("Bairro:", value=st.session_state.dados['bairro'], key="b1")
        st.session_state.dados['cidade'] = cc.text_input("Cidade:", value=st.session_state.dados['cidade'], key="cid1")
        st.session_state.dados['uf'] = cd.text_input("UF:", value=st.session_state.dados['uf'], max_chars=2, key="uf1")

    # 2. BLOCO DE EQUIPAMENTO (TRAVA DO FLUIDO)
    st.markdown("---")
    ...
    with st.expander("Detalhes Técnicos", expanded=True):
        ...
        with e1:
            st.session_state.dados['fabricante'] = st.selectbox("Fabricante:", sorted(["Carrier", "Daikin", "Fujitsu", "LG", "Samsung", "Trane"]), key="f1")
            st.session_state.dados['modelo'] = st.text_input("Modelo:", value=st.session_state.dados['modelo'], key="m1")

        with e3:
            # --- CORREÇÃO DO FLUIDO: O código agora lembra o que você escolheu ---
            lista_f = ["R410A", "R134a", "R22", "R32", "R290"]
            f_salvo = st.session_state.dados.get('fluido', 'R410A')
            f_idx = lista_f.index(f_salvo) if f_salvo in lista_f else 0

            st.session_state.dados['fluido'] = st.selectbox("Fluido:", lista_f, index=f_idx, key="fluid_v3")
            st.session_state.dados['capacidade'] = st.selectbox("Capacidade:", ["9.000", "12.000", "18.000", "24.000"], index=1, key="cap_v3")
            st.session_state.dados['tag_id'] = st.text_input("TAG:", value=st.session_state.dados['tag_id'], key="t1")
# ==============================================================================
