# ==============================================================================
# 0. FUNÇÕES DE APOIO E CALLBACKS (ADICIONADO PARA CORREÇÃO)
# ==============================================================================
def atualizar_fluido_callback():
    """Garante que o fluido e as pressões sejam travados no momento da escolha"""
    novo_fluido = st.session_state.key_fluido_selectbox
    st.session_state.dados['fluido'] = novo_fluido
    
    # Define pressões padrão na Aba 2 automaticamente ao trocar o gás
    if novo_fluido == "R22":
...
# 1. FUNÇÃO DA ABA 1: Identificação e Equipamento (VERSÃO CORRIGIDA)
# ==============================================================================
def renderizar_aba_1():
    ...
    with tab1:
        ...
        with col_data: st.session_state.dados['data'] = st.text_input("Data da Visita:", value=st.session_state.dados['data'], key="cli_data_v2")
        ...
        with st.expander("Detalhes Técnicos do Ativo", expanded=True):
            ...
            with e1:
                ...
                st.session_state.dados['fabricante'] = st.selectbox("Fabricante:", fab_list, index=fab_idx, key="eq_fab_v2")
                st.session_state.dados['modelo'] = st.text_input("Modelo:", value=st.session_state.dados['modelo'], key="eq_mod_v2")
                st.session_state.dados['linha'] = st.selectbox("Linha:", ["Residencial", "Comercial", "Industrial"], key="eq_lin_v2")
                st.session_state.dados['status_maquina'] = st.radio("Status:", ["🟢 Operacional", "🟡 Requer Atenção", "🔴 Parado"], horizontal=True, key="eq_stat_v2")

            with e2:
                st.session_state.dados['serie_evap'] = st.text_input("Nº Série (EVAP) *", value=st.session_state.dados['serie_evap'], key="eq_sevap_v2")
                st.session_state.dados['serie_cond'] = st.text_input("Nº Série (COND)", value=st.session_state.dados['serie_cond'], key="eq_scond_v2")
                st.session_state.dados['local_evap'] = st.text_input("Local da Evaporadora:", value=st.session_state.dados['local_evap'], key="eq_levap_v2")
                st.session_state.dados['local_cond'] = st.text_input("Local da Condensadora:", value=st.session_state.dados['local_cond'], key="eq_lcond_v2")

            with e3:
                st.session_state.dados['capacidade'] = st.selectbox("Capacidade:", ["9.000", "12.000", "18.000", "24.000", "30.000", "36.000", "48.000", "60.000"], key="eq_cap_v2")
                
                # --- CORREÇÃO DO FLUIDO (INDEX + CALLBACK) ---
                fluidos_lista = ["R410A", "R134a", "R22", "R32", "R290"]
                f_memoria = st.session_state.dados.get('fluido', 'R410A')
                f_idx = fluidos_lista.index(f_memoria) if f_memoria in fluidos_lista else 0
                
                st.selectbox(
                    "Fluido:", 
                    fluidos_lista, 
                    index=f_idx, 
                    key="key_fluido_selectbox",
                    on_change=atualizar_fluido_callback
                )
                
                st.session_state.dados['tipo_servico'] = st.selectbox("Tipo de Serviço:", ["Manutenção Preventiva", "Manutenção Corretiva", "Instalação", "Infraestrutura"], key="eq_serv_v2")
                st.session_state.dados['tag_id'] = st.text_input("TAG:", value=st.session_state.dados['tag_id'], key="eq_tag_v2")
        ...
        with col_btn:
            if st.button("✅ Confirmar Equipamento", use_container_width=True):
                # Força a atualização dos parâmetros caso o técnico não tenha mudado o selectbox
                atualizar_fluido_callback()
