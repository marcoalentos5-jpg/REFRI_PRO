...
def renderizar_aba_1():
    ...
    with tab1:
        ...
        # --- SEÇÃO EQUIPAMENTO (ESTRUTURA CORRIGIDA) ---
        col_titulo, col_data = st.columns([3, 1])
        with col_titulo: st.subheader("⚙️ Especificações do Equipamento")
        with col_data: st.session_state.dados['data'] = st.text_input("Data da Visita:", value=st.session_state.dados['data'], key="data_visita_v2")
        ...
        with st.expander("Detalhes Técnicos do Ativo", expanded=True):
            ...
            with e1:
                ...
                st.session_state.dados['linha'] = st.selectbox("Linha:", ["Residencial", "Comercial", "Industrial"], index=0, key="linha_sel_v2")
                st.session_state.dados['status_maquina'] = st.radio("Status:", ["🟢 Operacional", "🟡 Requer Atenção", "🔴 Parado"], horizontal=True, key="status_sel_v2")

            with e2:
                st.session_state.dados['serie_evap'] = st.text_input("Nº Série (EVAP) *", value=st.session_state.dados['serie_evap'], key="se_sel_v2")
                st.session_state.dados['serie_cond'] = st.text_input("Nº Série (COND)", value=st.session_state.dados['serie_cond'], key="sc_sel_v2")
                st.session_state.dados['local_evap'] = st.text_input("Local da Evaporadora:", value=st.session_state.dados['local_evap'], key="le_sel_v2")
                st.session_state.dados['local_cond'] = st.text_input("Local da Condensadora:", value=st.session_state.dados['local_cond'], key="lc_sel_v2")

            with e3:
                # --- AQUI ESTÁ O TRECHO QUE VOCÊ PROCURAVA ---
                ...
                st.session_state.dados['tipo_servico'] = st.selectbox(
                    ...
                    index=0,
                    key="serv_selectbox_aba1"
                )
                
                st.session_state.dados['tag_id'] = st.text_input("TAG:", value=st.session_state.dados['tag_id'], key="tag_input_aba1")
