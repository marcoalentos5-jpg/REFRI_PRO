...
def renderizar_aba_1():
    # O uso do expander ajuda a manter o layout limpo
    with st.expander("👤 Dados do Cliente e Endereço", expanded=True):
        ...
        # --- SEÇÃO ENDEREÇO (CORRIGIDA) ---
        ce1, ce2, ce3 = st.columns([1, 2, 1])
        
        # CEP com gatilho de busca
        cep_atual = ce1.text_input("CEP *", value=st.session_state.dados['cep'], key="cli_cep_v2")
    ...
    # --- SEÇÃO EQUIPAMENTO (CORRIGIDA PARA PERSISTÊNCIA) ---
    ...
    with st.expander("Detalhes Técnicos do Ativo", expanded=True):
        ...
        with e1:
            ...
            st.session_state.dados['fabricante'] = st.selectbox("Fabricante:", fab_list, index=fab_idx, key="fab_sel")
            
            st.session_state.dados['modelo'] = st.text_input("Modelo:", value=st.session_state.dados['modelo'], key="mod_sel")

        with e3:
            # FIX DO FLUIDO: Definimos a lista e buscamos o index salvo para travar a escolha
            fluidos_lista = ["R410A", "R134a", "R22", "R32", "R290"]
            f_atual = st.session_state.dados.get('fluido', 'R410A')
            f_idx = fluidos_lista.index(f_atual) if f_atual in fluidos_lista else 0
            
            st.session_state.dados['fluido'] = st.selectbox(
                "Fluido:", 
                options=fluidos_lista, 
                index=f_idx, 
                key="fluido_aba1" # Chave única para persistência
            )
            
            st.session_state.dados['capacidade'] = st.selectbox("Capacidade:", ["9.000", "12.000", "18.000", "24.000", "30.000", "36.000", "48.000", "60.000"], index=1, key="cap_sel")

# Funções técnicas mantidas abaixo...
