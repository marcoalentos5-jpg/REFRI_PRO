...
def diagnostico_avancado_bobina(v_med, a_med, q_reativa, freq=60):
    ...
    Fórmulas aplicadas: 
    1. XL = Q / I² (Reatância)
    2. L = XL / (2 * pi * f) (Indutância)
    """
    if a_med <= 0:
        return 0, 0
    
    # 1. Calcular a Reatância Indutiva (O 'obstáculo' em Ohms)
    # Reatância XL = Potência Reativa / Corrente ao quadrado
    reatancia_xl = q_reativa / (a_med ** 2)
    
    # 2. Calcular a Indutância (A 'peça' física em Henrys)
    # L = XL / (2 * pi * frequencia)
    indutancia_l = reatancia_xl / (2 * math.pi * freq)
    
    return round(reatancia_xl, 2), round(indutancia_l, 4)

# --- INTEGRAÇÃO NA INTERFACE DO STREAMLIT ---

# Supondo que você já calculou a 'q_reativa' no bloco anterior...
xl, l_henry = diagnostico_avancado_bobina(v_med, a_med, q_reativa)

with st.expander("🔍 Inspeção Interna do Enrolamento (Indutância)"):
    col_l1, col_l2 = st.columns(2)
    col_l1.metric("Reatância (XL)", f"{xl} Ω", help="Resistência magnética da bobina.")
    col_l2.metric("Indutância (L)", f"{l_henry} H", help="Valor físico da bobina. Se cair muito, há curto entre espiras.")
    
    # Lógica de Alerta baseada na 'Inércia' da bobina
    if xl < 5 and a_med > 10: # Exemplo de threshold para compressores médios
        st.warning("⚠️ **Alerta de Curto Parcial:** A reatância está muito baixa para esta corrente. O verniz do enrolamento pode estar degradado.")
