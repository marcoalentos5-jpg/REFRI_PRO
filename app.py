...
with tab_cad:
    ...
    email_cli = c7.text_input("✉️ E-mail", key="f_mail")

    st.markdown("---")
    st.subheader("📍 Endereço da Visita")
    e1, e2, e3, e4 = st.columns([0.8, 2, 0.5, 0.7])
    ...
    fluido = t7.selectbox("Fluido Refrigerante", ["R-410A", "R-32", "R-22", "R-134a", "R-404A"], key="f_gas")
    
    l1, l2, l3 = st.columns(3)
    loc_evap = l1.text_input("Localização Evap.", key="f_le")
    mod_evap = l2.text_input("Mod. Evap.", key="f_me")
    serie_evap = l3.text_input("Série Evap.", key="f_se")
    
    l4, l5, l6 = st.columns(3)
    loc_cond = l4.text_input("Localização Cond.", key="f_lc")
    mod_cond = l5.text_input("Mod. Cond.", key="f_mc")
    serie_cond = l6.text_input("Série Cond.", key="f_sc")

with tab_ele:
    st.subheader("⚡ Parte Elétrica")
    el1, el2, el3 = st.columns(3)
    v_rede = el1.number_input("Tensão de Rede (V)", value=220.0)
...
with tab_diag:
    ...
    medidas = st.text_area("Parecer Técnico e Recomendações", value=resumo_pre, height=150)
    
    if st.button("📄 Gerar Relatório MPN"):
        pdf = FPDF()
        pdf.add_page()
        ...
        pdf.set_x(12); pdf.cell(50, 4, f"WhatsApp: {whatsapp}", 0, 0); pdf.cell(50, 4, f"Celular: {celular}", 0, 0); pdf.cell(50, 4, f"Fixo: {tel_residencial}", 0, 1)
        ...
        # --- 3. ANÁLISE DE PARÂMETROS (TRIPARTIDA) ---
        pdf.set_y(pdf.get_y() + 6)
        pdf.set_font("Arial", 'B', 9); pdf.set_fill_color(220, 230, 241); pdf.set_text_color(0, 51, 102)
        ...
        st.download_button("⬇️ Baixar Relatório Profissional", data=pdf.output(dest="S").encode('latin-1'), file_name=f"MPN_{cliente}.pdf")
