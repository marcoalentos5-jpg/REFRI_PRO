...
with tab_diag:
    ...
    if tecnologia in ["Inverter", "VRF"] and sh_val > 15:
        analise_ia.append(f"🔍 [PERÍCIA] Sistema {tecnologia}: SH crítico. Possível falha de EEV ou falta de fluido.")
        medidas_ia.append("1. Verificar abertura da válvula de expansão eletrônica.")
    
    if "Congelando" in str(p_sel):
        analise_ia.append("🔍 [ENGENHARIA] Evidência de restrição física conforme literatura técnica.")
        medidas_ia.append("2. Realizar limpeza química de serpentinas.")

    if not analise_ia:
        analise_ia.append("✅ Parâmetros em conformidade com manuais técnicos.")
    ...
    exec_med = c_ia2.text_area("📋 Medidas Executadas", height=200)

    if st.button("📄 Gerar Relatório Profissional"):
        pdf = FPDF()
        pdf.add_page()
        
        # CABEÇALHO (REPLICAÇÃO EXATA DA IMAGEM)
        ...
        pdf.cell(50, 7, f" Data da visita: {data_v.strftime('%d/%m/%Y')}", 1, 1, 'L', True)
        pdf.set_font("Arial", '', 9); pdf.set_text_color(0)
        pdf.cell(190, 25, "", 1)
        pdf.set_xy(15, 45)
        pdf.cell(0, 5, clean(f"Cliente: {cliente}"), 0, 1)
        pdf.cell(0, 5, clean(f"Endereco: {logr}, {num} - {bairro}"), 0, 1)
        pdf.cell(0, 5, clean(f"Whats: {whatsapp} | Cel: {celular}"), 0, 1)

        # TABELA DADOS DO EQUIPAMENTO
        pdf.set_xy(10, 75)
        pdf.set_font("Arial", 'B', 10); pdf.cell(190, 7, " Dados do Equipamento", 1, 1, 'L', True)
        pdf.set_font("Arial", '', 9)
        pdf.cell(190, 25, "", 1)
        pdf.set_xy(15, 85)
        pdf.cell(63, 5, clean(f"Marca: {marca}"), 0); pdf.cell(63, 5, clean(f"Tecnologia: {tecnologia}"), 0); pdf.cell(64, 5, clean(f"Modelo: {modelo}"), 0, 1)
        pdf.cell(63, 5, clean(f"Sistema: {sistema}"), 0); pdf.cell(63, 5, clean(f"Fluido: {fluido}"), 0, 1)

        # SEÇÃO DIAGNÓSTICO (LAYOUT DA IMAGEM)
        pdf.set_xy(10, 115)
        pdf.set_font("Arial", 'B', 10); pdf.cell(190, 7, " Parecer Tecnico / Diagnostico", 1, 1, 'L', True)
        pdf.set_font("Arial", '', 9); pdf.cell(190, 40, "", 1)
        pdf.set_xy(15, 125)
        pdf.cell(0, 5, clean(f"Problemas: {', '.join(p_sel)}"), 0, 1)
        pdf.multi_cell(180, 5, clean(f"IA: {'. '.join(analise_ia)}"))
        pdf.multi_cell(180, 5, clean(f"Observacoes: {obs_tec}"))
        pdf.multi_cell(180, 5, clean(f"Executado: {exec_med}"))
