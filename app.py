...
# --- 5. CRIAÇÃO DAS ABAS (IMPORTANTE: PRECISA VIR ANTES DOS BLOCOS 'WITH') ---
...
with tab_ele:
    ...
    e1, e2 = st.columns(2)
    with e1:
        v_nom = st.selectbox("Tensão Nominal (V)", [127, 220, 380, 440], index=1)
        v_med = st.number_input("Tensão Medida (V)", value=float(v_nom))
    with e2:
        a_nom = st.number_input("Corrente Nominal RLA (A)", value=5.0)
...
with tab_diag:
    ...
    if a_med == 0 and p_suc == 120.0:
        st.warning("Insira os dados técnicos nas abas anteriores para gerar o veredito.")
    else:
        if sh < 5: st.error("🚨 SH BAIXO: Risco de retorno de líquido ao compressor.")
        elif sh > 12: st.error("🚨 SH ALTO: Falta de fluido ou baixo rendimento.")
    ...
    if st.button("💾 Finalizar Diagnóstico"):
        st.balloons()
        st.success(f"Laudo técnico de {cliente} preparado para exportação!")
