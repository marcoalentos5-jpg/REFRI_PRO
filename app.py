...
with tab_diag:
    ...
    with col_prob:
        ...
        with st.container():
            ...
            p1 = st.checkbox("Falta de Fluido Refrigerante / Vazamento")
            p2 = st.checkbox("Excesso de Fluido Refrigerante")
            p3 = st.checkbox("Obstrução no Dispositivo de Expansão")
            p4 = st.checkbox("Ineficiência de Compressão (Mecânica)")
            p5 = st.checkbox("Contaminação por Umidade / Incondensáveis")
            p6 = st.checkbox("Falha na Ventilação (Evap/Cond)")
            p7 = st.checkbox("Instabilidade de Tensão / Falha Elétrica")
            p8 = st.checkbox("Sensores Descalibrados / Curto")
            st.markdown('</div>', unsafe_allow_html=True)
    with col_obs:
        st.subheader("📝 Observações do Técnico")
        obs_tecnico = st.text_area("", placeholder="Digite aqui o parecer técnico detalhado...", height=282, label_visibility="collapsed")
    st.markdown("---")
    col_prop, col_tom = st.columns(2)
    with col_prop:
        st.subheader("🔧 Propostas")
        st.markdown('<div class="status-box">', unsafe_allow_html=True)
        if sh_val > 12: st.warning("🎯 Realizar teste de estanqueidade e recarga.")
        if sh_val < 5: st.warning("🎯 Verificar possível retorno de líquido ao compressor.")
        if sc_val > 12: st.warning("🎯 Limpeza química da condensadora ou ajuste de carga.")
        if sc_val < 3: st.warning("🎯 Verificar restrições na linha de líquido.")
        st.markdown('</div>', unsafe_allow_html=True)
    with col_tom:
        st.subheader("📋 Medidas")
        st.markdown("**Tomadas**")
        tomadas_input = st.text_area("", placeholder="Medidas executadas...", key="tomadas_diag", height=230, label_visibility="collapsed")

    # --- 4. RELATÓRIO PDF (FORMATO DA IMAGEM CONGELADO) ---
