...
with tab_diag:
    ...
    if st.button("📄 Gerar Relatório Profissional"):
        ...
        pdf.cell(30, 6, "Cliente:", ln=0); pdf.set_font("Arial", '', 9); pdf.cell(80, 6, clean(cliente), ln=0)
        pdf.set_x(120) 
        pdf.set_font("Arial", 'B', 9); pdf.cell(30, 6, "CPF/CNPJ:", ln=0); pdf.set_font("Arial", '', 9); pdf.cell(50, 6, clean(doc_cliente), ln=1)
        pdf.set_font("Arial", 'B', 9); pdf.cell(30, 6, "Endereco:", ln=0); pdf.set_font("Arial", '', 9); pdf.cell(80, 6, clean(endereco), ln=0)
        pdf.set_x(120) 
        pdf.set_font("Arial", 'B', 9); pdf.cell(30, 6, "Bairro/CEP:", ln=0); pdf.set_font("Arial", '', 9); pdf.cell(50, 6, f"{clean(bairro)} / {clean(cep)}", ln=1)
        
        pdf.ln(2)
        pdf.set_font("Arial", 'B', 9); pdf.cell(30, 6, "Equipamento:", ln=0); pdf.set_font("Arial", '', 9); pdf.cell(80, 6, f"{clean(fabricante)} {clean(cap_digitada)} BTU - {clean(local_eq)}", ln=0)
        pdf.set_x(120)
        pdf.set_font("Arial", 'B', 9); pdf.cell(30, 6, "Data:", ln=0); pdf.set_font("Arial", '', 9); pdf.cell(50, 6, f"{data_visita}", ln=1)
        pdf.ln(5)

        # --- 2. DADOS TÉCNICOS ---
        draw_header("2. Analise de Campo (Termodinamica e Eletrica)")
        pdf.set_font("Arial", 'B', 9)
        pdf.cell(45, 6, f"Pressao Succao: {p_suc} PSI", ln=0)
        pdf.cell(45, 6, f"Pressao Liquido: {p_liq} PSI", ln=0)
        pdf.cell(45, 6, f"Gas: {clean(fluido)}", ln=1)
        pdf.cell(45, 6, f"Superaq. (SH): {sh} K", ln=0)
        pdf.cell(45, 6, f"Sub-resf. (SC): {sc} K", ln=1)
        pdf.cell(45, 6, f"Tensao: {v_med} V", ln=0)
        pdf.cell(45, 6, f"Corrente Med: {a_med} A", ln=1)
        pdf.ln(5)
        ...
        pdf.set_font("Arial", '', 9); pdf.multi_cell(0, 5, clean(obs_raw))
        pdf.ln(2)
        pdf.set_font("Arial", 'B', 9); pdf.cell(0, 6, "Medidas Propostas:", ln=True)
        pdf.set_font("Arial", '', 9); pdf.multi_cell(0, 5, clean(ia_raw))

        # --- EXPORTAR ---
        pdf_out = pdf.output(dest='S')
        pdf_bytes = bytes(pdf_out) if not isinstance(pdf_out, str) else pdf_out.encode('latin-1', 'replace')

        st.download_button(
            label="⬇️ Baixar Relatório em PDF",
            data=pdf_bytes,
            file_name=f"Relatorio_{clean(cliente)}.pdf",
