...
# --- ABA 03: TERMODINÂMICA (CORREÇÃO DE INDENTAÇÃO) ---
with tab3:
    st.subheader("❄️ Análise do Ciclo Frigorífico")

    # 1. Inicialização Segura (Somente chaves da Aba 3)
    chaves_termo = {
        'p_suc': 0.0, 'p_desc': 0.0, 't_suc': 0.0, 't_liq': 0.0, 't_insufla': 12.0
    }
    ...
    # 2. Entrada de Dados
    with st.expander("🌡️ Pressões e Temperaturas", expanded=True):
    ...
    # 3. Cálculos de SH e SR
    st.divider()
    c_sh, c_sr = st.columns(2)
    
    # Cálculo Superaquecimento (SH) - Exemplo R410A
    t_sat_evap = (p_suc / 3.0) - 10 if p_suc > 0 else 0
    sh = t_suc - t_sat_evap
    c_sh.metric("Superaquecimento (SH)", f"{sh:.1f} K", "5 a 8K Ideal")

    # Cálculo Sub-resfriamento (SR) - Exemplo R410A
    t_sat_cond = (p_desc / 10.0) + 15 if p_desc > 0 else 0
    sr = t_sat_cond - t_liq
    c_sr.metric("Sub-resfriamento (SR)", f"{sr:.1f} K", "3 a 7K Ideal")

    # Atualiza dados no dicionário principal sem mexer na Aba 1
    st.session_state.dados.update({'p_suc': p_suc, 'p_desc': p_desc, 't_suc': t_suc, 't_liq': t_liq})
