...
with tab_diag:
    ...
    with col_prop_ia:
        ...
        diag_ia = f"Análise: SH {sh_val}K | SC {sc_val}K. Sistema {tecnologia}."
        st.info(diag_ia)
    ...
    if st.button("📄 Gerar Relatório Profissional"):
        ...
        pdf.set_font("Arial", '', 8); pdf.set_text_color(0); pdf.cell(190, 15, clean(f"Cliente: {cliente}\nEndereço: {nome_logr}, {numero} - {bairro}"), 1, 1)
        pdf.set_font("Arial", 'B', 9); pdf.cell(190, 7, " Dados do Equipamento", 1, 1, 'L', True)
        pdf.set_font("Arial", '', 8); pdf.cell(190, 15, clean(f"Marca: {fabricante} | Modelo: {modelo_eq} | Fluido: {fluido}"), 1, 1)
