...
with tab_diag:
    ...
    with col_wa:
        ...
        st.markdown(f'<a href="{url_wa}" target="_blank" style="text-decoration:none;"><button style="width:100%; background-color:#25D366; color:white; border:none; padding:15px; border-radius:8px; font-weight:bold; cursor:pointer;">📲 ENVIAR VIA WHATSAPP</button></a>', unsafe_allow_html=True)
    ...
    with col_pdf:
        ...
        # MEDIÇÕES (ELÉTRICA + TERMO)
        pdf.ln(2); pdf.set_font("helvetica", "B", 9); pdf.cell(190, 7, " PARÂMETROS ELÉTRICOS E TERMODINÂMICOS", border="LR", ln=True, fill=True)
        ...
        # PARECER TÉCNICO FINAL (CORREÇÃO DO CAMPO OBS)
        ...
        # Correção aqui: multi_cell permite que o texto longo pule linha no PDF sem sumir
        pdf.set_font("helvetica", "I", 8)
        pdf.multi_cell(190, 7, f" Recomendacoes: {obs_final}", border=1)
