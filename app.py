...
# FUNÇÃO DE BUSCA DE CEP (ALINHADA PARA EVITAR ESMAECIMENTO)
def buscar_cep_estavel():
    cep_input = st.session_state.cep_digita
...
# ==============================================================================
# 1. FUNÇÃO DA ABA 1: Identificação e Equipamento (CÓDIGO COMPLETO E PRESERVADO)
# ==============================================================================
def renderizar_aba_1():
    ...
    with tab1:
        ...
        with st.expander("👤 Dados do Cliente e Endereço", expanded=True):
            ...
            st.session_state.dados['tel_fixo'] = cx2.text_input("Telefone Fixo:", value=st.session_state.dados['tel_fixo'], key="cli_fixo")
            st.session_state.dados['email'] = cx3.text_input("E-mail:", value=st.session_state.dados['email'], key="cli_email")

            st.markdown("---")
            ce1, ce2, ce3 = st.columns([1, 2, 1])
            # Alinhamento do CEP com on_change para estabilidade total
            ce1.text_input("CEP *", value=st.session_state.dados['cep'], key="cep_digita", on_change=buscar_cep_estavel)
            ...
            col_uf = st.columns([1])
            with col_uf[0]:
                st.session_state.dados['uf'] = st.text_input("UF:", value=st.session_state.dados['uf'], key="cli_uf")
            # -----------------------------------------------

        # --- SEÇÃO EQUIPAMENTO ---
        col_titulo, col_data = st.columns([3, 1])
        with col_titulo: st.subheader("⚙️ Especificações do Equipamento")
        with col_data: st.session_state.dados['data'] = st.text_input("Data da Visita:", value=st.session_state.dados['data'], key="vis_data")
        ...
        with st.expander("Detalhes Técnicos do Ativo", expanded=True):
            ...
            with e1:
                ...
                st.session_state.dados['fabricante'] = st.selectbox("Fabricante:", fab_list, index=fab_idx, key="eq_fab")
                st.session_state.dados['modelo'] = st.text_input("Modelo:", value=st.session_state.dados['modelo'], key="eq_mod")
                st.session_state.dados['linha'] = st.selectbox("Linha:", ["Residencial", "Comercial", "Industrial"], index=0, key="eq_lin")
                st.session_state.dados['status_maquina'] = st.radio("Status:", ["🟢 Operacional", "🟡 Requer Atenção", "🔴 Parado"], horizontal=True, key="eq_stat")
            ...
            with e3:
                st.session_state.dados['capacidade'] = st.selectbox("Capacidade:", ["9.000", "12.000", "18.000", "24.000", "30.000", "36.000", "48.000", "60.000"], index=1, key="eq_cap")
                st.session_state.dados['fluido'] = st.selectbox("Fluido:", ["R410A", "R134a", "R22", "R32", "R290"], index=0, key="eq_flu")
                st.session_state.dados['tipo_servico'] = st.selectbox("Tipo de Serviço:", ["Manutenção Preventiva", "Manutenção Corretiva", "Instalação", "Infraestrutura"], index=0, key="eq_serv")
